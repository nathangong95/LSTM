...
def particle_filter_with_a(joints, heatmap_path, Q, GT, num_particles=100, alpha=0.5):
    """
        joints: 3000*7*2
        GT: 3000*7*2
        Q: 4*4*7
    ...
    for j in range(0, 7):
        ...
        for i in range(joints.shape[0]):
            ...
            if i == 0:
                ...
                weight = []
                for part in particles:
                    if part[0, 0] == 250 or part[1, 0] == 250 or part[0, 0] == 5 or part[1, 0] == 5:
                        weight.append(0)
                    else:
                        weight.append(
                            (1 - alpha) * heatmap[int(part[0, 0]), int(part[1, 0])] + alpha * 1.0 / (255 * 255))  #
                weight = weight / sum(weight)
                sum_x = 0
                sum_y = 0
...
# for i in range(8):
joints_particlefilter = particle_filter_with_a(joints, heatmap_path, Q, GT, num_particles=1000, alpha=0.1)
np.save('joints_particle'+str(0.1)+'.npy', joints_particlefilter)
joints_particlefilter = particle_filter_with_a(joints, heatmap_path, Q, GT, num_particles=1000, alpha=1)
np.save('joints_particle'+str(1)+'.npy', joints_particlefilter)
